...
class Live(object):
  def __init__(self, args_data):
    ...
    # ----------------------------------------
    # 左右停止数组
    # self.left_stop_index_arr = []
    # self.right_stop_index_arr = []
    ...
    # self.source_output = 0                            # 源视频
    # self.dist_output = 0                              # 推理视频
  ...
  def timer_manager(self,timing):
    ...
    if self.time_overtime >= 10:
      ...
      r8.write_queue("reasoningEndList", str(self.camera))
      time.sleep(2)
  ...
  def push_frame(self):
    ...
    while True:
      if self.frame_queue.empty() != True:
        ...
        self.now_index += 1
        if self.now_index % self.skip_frame_index == 0:
          # 没找到板，推理
          if self.result_dn_data['board_bool'] != 1:
          ...
          if len(self.first_frame) != 0 and len(frame) != 0:
            # 获取当前帧的切片压缩图
            ...
            temp_frame[pt1[1]:pt2[1], pt1[0]:pt2[0]] = 255
            # 2 剪切放大区域

            pt1 = (int(self.t_board_arr['xmin']), int(self.t_board_arr['ymin']))
            pt2 = (int(self.t_board_arr['xmax']), int(self.t_board_arr['ymax']))
            check_frame = temp_frame[pt1[1]:pt2[1], pt1[0]:pt2[0]]
            
            temp_size = check_frame.shape

            frame_3, frame_4, frame_5, frame_6 = utils.cut_img(check_frame, temp_size)
            (f3_score, f1_dif) = compare_ssim(self.first_3, frame_3, full=True)
            (f4_score, f1_dif) = compare_ssim(self.first_4, frame_4, full=True)
            (f5_score, f1_dif) = compare_ssim(self.first_5, frame_5, full=True)
            (f6_score, f1_dif) = compare_ssim(self.first_6, frame_6, full=True)
            print("socre",f3_score,f4_score,f5_score,f6_score)
            if f3_score < self.threshold_ssim or f4_score < self.threshold_ssim or f5_score < self.threshold_ssim or f6_score < self.threshold_ssim:
              self.pass_frame_num += 1
              # self.result_dn_data['smoke_change'] = 1
              frame = cv2.putText(frame, 'f1: ' + str(round(f3_score, 2)), (self.size[0]-200, self.size[1]-120), cv2.FONT_HERSHEY_SIMPLEX, 1,[0,0,255], 2)
              frame = cv2.putText(frame, 'f2: ' + str(round(f4_score, 2)), (self.size[0]-200, self.size[1]-90), cv2.FONT_HERSHEY_SIMPLEX, 1,[0,0,255], 2)
              frame = cv2.putText(frame, 'f3: ' + str(round(f5_score, 2)), (self.size[0]-200, self.size[1]-60), cv2.FONT_HERSHEY_SIMPLEX, 1,[0,0,255], 2)
              frame = cv2.putText(frame, 'f4: ' + str(round(f6_score, 2)), (self.size[0]-200, self.size[1]-30), cv2.FONT_HERSHEY_SIMPLEX, 1,[0,0,255], 2)
              frame = cv2.putText(frame, 'f: ' + str(self.pass_frame_num), (self.size[0]-200, self.size[1]-150), cv2.FONT_HERSHEY_SIMPLEX, 1,[0,0,255], 2)
        ...
        if not self.first_frame_std and len(rect_img):
          utils.write_log(str(self.camera), "push_frame 准备执行service.first_frame_std")
          try:
        ...
        if self.frame_index and self.now_index % self.darkness_skip_index == 0:
          ...
          # 记录第一帧
          if self.first_frame == [] and self.result_dn_data['board_bool'] == 1:
            print(len(pure_frame))
            self.first_frame = pure_frame
            temp_frame = pure_frame.copy()

            self.t_board_arr = dn.scaleBox(self.filedata, self.size)
            
            print("self.filedata",self.filedata)
            print("self.t_board_arr",self.t_board_arr)
            # 1 涂白白板
            pt1 = (self.filedata['xmin'], self.filedata['ymin'])
            pt2 = (self.filedata['xmax'], self.filedata['ymax'])
            temp_frame[pt1[1]:pt2[1], pt1[0]:pt2[0]] = 255
            ...
            # cv2.imwrite("check_frame.jpg", check_frame)
            ...
            self.first_3, self.first_4, self.first_5, self.first_6 = utils.cut_img(check_frame, temp_size)

            utils.write_log(str(self.camera), "push_frame 将检出框放大，并切成4份")
          ...
          # print("rect_img_copy_sum: ", rect_img_copy_sum, now_rect_img_sum)
          # 每一帧的平均黑度
          ...
          self.data = r.get_values(self.camera)
          self.data['blackness'] = blackness
          r.set_values(self.camera ,self.data)
          now_blackness = utils.darkness_level(rect_img_sum)
          # 获取最大等级
          # print("now_blackness: ", now_blackness, self.result_dn_data['level'])
          ...
          # print("blackness: ", blackness, now_blackness)
          self.diff_black_arr.append(rect_img_sum)
          if len(self.diff_black_arr) > 250:
            self.diff_black_arr = self.diff_black_arr[1:]

          temp_black_arr = self.diff_black_arr.copy()
          # print("black_arr: ", diff_black_arr)

          temp_black_arr.extend([0 for i in range(250 - len(temp_black_arr))])

          # frame = utils.draw_axis(frame, temp_black_arr, (80, 430))  # 上面的图表80,430=坐标位置
          # frame = utils.draw_axis(frame, self.histb, (80, self.size[1]-100), [], int(max(self.histb)))

          # 画波顶蓝线
          # frame = utils.drawLine(frame, self.peak_ind, len(self.histb), (40, self.size[1]-100))  # 下面图表中的两个蓝线

          # 左波谷（两条红线）
          # frame = cv2.line(frame, (40 + int((486/255) * self.left_stop_index), self.size[1]-100),(40 + int((486/255) * self.left_stop_index), self.size[1]-200), (0, 0, 255))
          # 右波谷
          # frame = cv2.line(frame, (40 + int((486/255) * self.right_stop_index), self.size[1]-100),(40 + int((486/255) * self.right_stop_index), self.size[1]-200), (0, 0, 255))
          ...
          # print("33333: ", fillPointXmin, fillPointXmax, fillPointYmin, fillPointYmax, len(rect_img), len(rect_img[0]))
          if fillPointXmin > 0 and fillPointXmax < self.size[0] and \
            fillPointYmin > 0 and fillPointYmax < self.size[1]:
            frame[fillPointYmin:fillPointYmax, fillPointXmin:fillPointXmax] = rect_img

          # 画框
          # if len(self.ring_detections):
          if len(self.filedata):
            frame = cv2.rectangle(frame, (self.filedata['xmin'], self.filedata['ymin']), (self.filedata['xmax'], self.filedata['ymax']), (0,0,255), 2)
            frame = dn.drawScaleBox(frame, self.filedata, self.size)  # 画扩大的框
