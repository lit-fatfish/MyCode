...
class EdgeDetection(object):  # FrogEyes
    def __init__(self, img, roi_pts):
        ...
        self.high_light_roi_mat = cv2.fillPoly(np.ones(self.roi_mat.shape, dtype=np.float) * 0.25,self.roi_pts, (1.0, 1.0, 1.0), )  # highlight the roi
    ...
    def get_polygon_contours(self, img, img_back):
        ...
        if np.max(thresh) == 0:  # have not different
            ...
            # thresh, contours = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            ...
            approxs = [approx for approx in approxs if len(approx) > self.min_side_num and cv2.arcLength(approx, True) > self.min_poly_len]
            contours = approxs
        return contours
...
def run():
    ...
    processes = [mp.Process(target=queue_img_put, args=(queue, user_name, user_pwd, camera_ip)),mp.Process(target=queue_img_get, args=(queue, camera_ip))]
